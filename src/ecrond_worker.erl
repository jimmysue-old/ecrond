...
-record(state, {
          name :: term(),
          schedule :: term(),
          task :: {Node, Module::atom(), Fun::function(), Args::list()} | mfa() | {Node, Fun::function()} | function(),
...
state(Pid) ->
    call(Pid, state).
alter_schedule(Pid, Schedule) ->
    call(Pid, {alter_schedule, Schedule}).
alter_task(Pid, Task) ->
    call(Pid, {alter_task, Task}).
...
handle_info({left, Time}, #state{type = Type} = State) 
                                when Type =:= interval orelse Type =:= once ->
    send_self_after(Time),
    {noreply, State};
handle_info({left, _Time}, #state{type = cron, schedule = Schedule} = State) ->
    set_trigger(Schedule),
    {noreply, State};

handle_info({'EXIT', Pid, normal}, #state{type = Type, schedule = Schedule, processor = Pid} = State) ->
    case Type of
        once ->
            {stop, normal, State};
        _ ->
            set_trigger(Schedule),
            {noreply, State#state{processor = undefined, state = waiting}}
    end;
handle_info({'EXIT', Pid, Reason}, #state{processor = Pid} = State)->
    {stop, Reason, State};
...
set_trigger({interval, MilliSeconds}) ->
    send_self_after(MilliSeconds);
set_trigger({cron,_Specs} = Schedule)->
